...
public class SubGraphsRetrieval implements ActionListener {
	static JButton button;
	static JTextField text;
	static String query;
	static JTextArea userWindow;
	...
	public void actionPerformed(ActionEvent evt)  {
        if ( evt.getSource() == button ) {
        	query=text.getText();
        	userWindow.setText(null);
        	change=false;
        	
        this.retrievingSubGraphs();
        	text.setText("");
        }
        }
	
	
	//L1; L2; L3 lists created
	
	public void retrievingSubGraphs()
	{
		try
    	{
		 ...
		 try {
			...
			//Creating  Edge Lists
			 ...
			 //Putting again into lists
				...
				//Creating adjacency Lists without unique subgraphs
				
				
		         ArrayList<ArrayList>latestFullMappings=new ArrayList<ArrayList>();
								
				//System.out.println(newFullMappings.size());
				     ...
				     for(int j=0;j<subgraph.size();j++)
				     {
				    	// System.out.println("Edge"+" "+j);
				    	 HashMap<ArrayList,String>edge=subgraph.get(j);
				    		...
				    		// System.out.println(edge.get(k).entrySet().iterator().next());
			...
			*/
			...
			File filess = new File("D://Project/SubgraphsNew2.doc");
				...
				FileWriter fwss = new FileWriter(filess.getAbsoluteFile());
				BufferedWriter bwss = new BufferedWriter(fwss);
			...
			//	File file=new File("D://Subgraphs.doc");
		   	...
		   	for(int i=0;i<Subgraphs.size()-1;i++)
			{
		       ...
		       File files = new File("D://Project/SubgraphsNew1.doc");
				...
				FileWriter fws = new FileWriter(files.getAbsoluteFile());
		       ...
		       File file = new File("D://Project/Subgraphs.doc");
	...
	System.out.println("Prathu");	       
		      ...
		      // Subgraphs=null;
		       ...
		       for(int i=0;i<RankedGraphs1.size();i++)
		       {
		    	  ...
		    	  if(subgraphh2.size()==1 && subgraphh.size()>1)
		    	  {
		       ...
		       File file1 = new File("D://Project/RankedResults.doc");
				...
				int dd=0;
				 for(int i=0;i<RankedGraphs1.size();i++)
			       ...
			       File file1a = new File("D://Project/Top30RankedResults.doc");
					...
					FileWriter fwnewa = new FileWriter(file1a.getAbsoluteFile());
					BufferedWriter bwnewa = new BufferedWriter(fwnewa);
					 ...
					 for(int i=0;i<RankedGraphs1.size();i++)
						{
							...
							bwnewa.newLine();
							bwnewa.write("Subgraph" +dda );
							bwnewa.newLine();
							userWindow.append("\n");
							userWindow.append("Subgraph"+dda+"\n");
	...
	public static void main(String[] args) {
		SubGraphsRetrieval retrieve=new SubGraphsRetrieval();
		...
		userWindow.setRows(30);
		 userWindow.setColumns(50);
       userWindow.setEditable(false);
       userWindow.setFont(new Font("URW Chancery L", Font.BOLD,16));
       userWindow.setBackground(new Color(218, 217, 199));
       userWindow.setEditable(false);
       userWindow.setLineWrap(true);
       userWindow.setWrapStyleWord(true);
       JScrollPane sp = new JScrollPane(userWindow, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
